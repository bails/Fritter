...
function show_posts($userid, $limit=0, $page=1){//change userid to username in fritter
	...
	$sql = "select P.id, U.username, P.body, P.stamp, I.pURL from posts P LEFT JOIN users U ON P.user_id = U.id LEFT JOIN ".TBL_USER_IMAGES." I ON U.id = I.user_id where P.user_id in ($user_string) order by P.stamp desc $extra";
